    ...
    def return_fixed_name(self, child_other):
        ...
        try:
            if type(child_other) is Folder:
                self.unique_folder_names[child_other.name] = self.unique_folder_names[
                    child_other.name
                ]
            if type(child_other) is File:
                self.unique_file_names[child_other.name] = self.unique_file_names[
                    child_other.name
                ]
        except:
            if type(child_other) is Folder:
    ...
    def action_path(self, value, folder, current_path=[], action="Add"):
        ...
        if len(current_path) == 0:
            ...
            if action == "Add":
                ...
                if type(value) is File and os.path.isfile(
                    os.path.join(value.filePath, value.name)
                ):
                    value = folder.return_fixed_name(value)
                    folder.contains.append(value)
        ...
        if len(current_path) > 0:
            ...
            folder.contains[current_path[0]] = self.action_path(
                value, folder.contains[current_path[0]], current_path[1:]
            )
            return folder
                    ...
                    if no_payload == False:
                        ...
                        data_file += (
                            ...
                            + str(
                                os.path.getsize(
                                    os.path.join(
                                        something.file_path, something.real_name
                                    )
                                )
                            ).encode()
                            + b"}"
                        ...
                        data_file += str(
                            os.path.getsize(
                                os.path.join(something.file_path, something.real_name)
                            )
                        ).encode()
                    if no_payload == False:
                        data_file += open(
                            something.file_path + "/" + something.real_name, "rb"
                        ).read()
                except:
                    data_file = b":|{0}"
                    logging.debug(
                        "\nERROR: Unable to package file, "
                        + something.name
                        + ", skipping.\n"
                    )
            ...
            if read_byte == b"<":  # working with folder.
                ...
                while True:
                    ...
                    if read_byte != b">":
                        ...
                        if temp[0:1] != b"/":
                            logging.debug(
                                "Folder " + temp.decode("utf-8") + " detected."
                            )

                            if root_folder_created == True:
                                os.mkdir(
                                    os.path.join(output_path, temp.decode("utf-8"))
                                )
                        ...
                        if temp[0:1] == b"/":
                            logging.debug(
                                "Folder termination detected. Moving one directory up."
                            )
                            os.chdir("..")
                            output_path = os.getcwd()
                            temp = b""
                ...
                while True:
                    ...
                    if read_byte != b"|":
                        ...
                        if not os.path.isdir(
                            ...
                            open(
                                os.path.join(output_path, temp.decode("utf-8")), "wb"
                            ).close()
            ...
            if read_byte == b"{":
                ...
                while True:
                    ...
                    if read_byte != b"}":
                        ...
                        with open(
                            os.path.join(output_path, current_file), "wb"
                        ) as write_file:
