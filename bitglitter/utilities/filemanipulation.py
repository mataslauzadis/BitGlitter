...
def compress_file(input_file, output_file, remove_input=True):
    ...
    with open(input_file, "rb") as process_in:
        with open(output_file, "wb") as process_out:
            with gzip.GzipFile(
                None, "wb", fileobj=process_out, mtime=0.0
            ) as process_out:
    ...
    with open(output_file, "wb") as file_out:
        with open(input_file, "rb") as file_in:
            ...
            file_out.write(
                _encrypt_bytes(
                    _derive_key(
                        encryption_key, salt, scrypt_n, scrypt_r, scrypt_p, backend
                    ),
                    initialization_vector_aes,
                    file_in.read(),
                    backend,
                )
            )
...
def _encrypt_bytes(key, initialization_vector, data, backend):
    ...
    return (
        encryptor.update(padder.update(data) + padder.finalize()) + encryptor.finalize()
    )
...
def _decrypt_bytes(key, initialization_vector_aes, data, backend):
    ...
    return (
        unpadder.update(decryptor.update(data) + decryptor.finalize())
        + unpadder.finalize()
    )
...
def _derive_key(password, salt, scrypt_n, scrypt_r, scrypt_p, backend):
    ...
    kdf = Scrypt(
        salt=salt, length=32, n=2 ** scrypt_n, r=scrypt_r, p=scrypt_p, backend=backend
    )
...
def refresh_working_folder(active_path):
    ...
    if os.path.isdir(active_path):
        ...
        logging.debug(
            f"active_path folder '{active_path}' already exists.  Deleting..."
        )
