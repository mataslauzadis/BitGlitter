from bitglitter.config.constants import (
    BG_VERSION,
    WRITE_PATH,
    SCRYPT_N_DEFAULT,
    SCRYPT_R_DEFAULT,
...
def write(  # Basic setup
    ...
    write_protocol = protocol_handler.return_write_protocol(
        "1"
    )  # will be dynamic when there are new protocols.

    # Are all parameters acceptable?
    write_protocol.verify_write_parameters(
        file_list,
        stream_name,
        stream_description,
        output_path,
    ...
    pre_process = write_protocol.pre_processing(
        active_path,
        file_list,
        encryption_key,
        file_mask_enabled,
    ...
    renderHandler = RenderHandler(
        ...
        block_width,
        header_palette_id,
        pre_process.stream_sha,
        pre_process.size_in_bytes,
        compression_enabled,
        encryption_key != "",
        file_mask_enabled,
        pre_process.date_created,
        stream_palette_id,
        BG_VERSION,
        stream_name,
        stream_description,
        pre_process.post_encryption_hash,
        pixel_width,
        output_mode,
        output_path,
        frames_per_second,
        active_path,
        pre_process.pass_through,
        output_name,
    )
