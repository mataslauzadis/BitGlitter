...
class RenderHandler:
    ...
    def __init__(
        ...
        self.initializer_palette_dict = ValuesToColor(
            self.initializer_palette, "initializer_palette"
        )

        self.protocol.begin_session_process(
            self.active_path,
            self.pass_through,
        ...
        config.stats_handler.write_update(
            (
                (self.protocol.total_frames - 1)
                * (self.block_height * self.block_width)
                + self.protocol.remainder_blocks
            ),
            self.protocol.total_frames,
            self.size_in_bytes,
        )
