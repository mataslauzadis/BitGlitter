...
def decode_stream_header_ascii_compressed(
    bitstream, custom_color_enabled, encryption_enabled
):
    ...
    logging.debug(
        f"ASCII header byte size inputted to read function: {int(len(bitstream) / 8)} B"
    )
    ...
    return (
        ...
        custom_color_date_created,
        custom_color_palette,
        post_compression_sha,
    )
...
def decode_stream_header_binary_preamble(bitStream):
    ...
    return (
        ...
        compression_enabled,
        encryption_enabled,
        file_masking_enabled,
        is_custom_palette,
        date_created,
        stream_palette_id,
        ascii_header_compressed_in_bytes,
    )
