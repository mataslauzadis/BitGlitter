...
from bitglitter.palettes.paletteutilities import (
    palette_grabber,
    ColorsToValue,
    _validate_and_add_palette,
)
from bitglitter.read.decoderassets import (
    minimum_block_checkpoint,
    read_frame_header,
    read_initializer,
    validate_payload,
)
...
class Decoder:
    ...
    def __init__(
        ...
        self.frame_handler = FrameHandler(
            self.initializer_palette, self.initializer_palette_dict
        )
    ...
    def decode_image(self, file_to_input):
        ...
        if (
            self.config_object.assembler.save_dict[
                self.stream_sha
            ].stream_header_ascii_complete
            == True
            and self.config_object.assembler.save_dict[
                self.stream_sha
            ].stream_palette_read
            == False
        ):
    ...
    def decode_video_frame(self, active_frame):
        ...
        if (
            self.stream_header_cleared == False
        ):  # We are still on header_palette frames.
    ...
    def _first_frame_setup(self):
        ...
        self.checkpoint_passed = minimum_block_checkpoint(
            self.block_height_override,
            self.block_width_override,
            self.frame_width,
            self.frame_height,
        )
        ...
        self.block_height, self.block_width, self.pixel_width = frame_lock_on(
            self.active_frame,
            self.block_height_override,
            self.block_width_override,
            self.frame_width,
        ...
        self.frame_handler.update_scan_geometry(
            self.block_height, self.block_width, self.pixel_width
        )
    ...
    def _frame_validation(self, palette_type):
        ...
        (
            self.frameHeaderBits,
            self.carry_over_bits,
        ) = self.frame_handler.return_frame_header(palette_type)

        (
            self.stream_sha,
            self.frame_sha,
            self.frame_number_of_stream,
            self.blocks_to_read,
        ...
        if (
            self.config_object.assembler.check_if_frame_needed(
                self.stream_sha, self.frame_number_of_stream
            )
            == False
        ):
    ...
    def _payload_process(self, palette_type):
        ...
        self.carry_over_bits.append(
            self.frame_handler.return_remainder_payload(palette_type)
        )
        ...
        self.config_object.assembler.accept_frame(
            self.stream_sha,
            self.frame_payload,
            self.frame_number_of_stream,
            self.scrypt_n,
    ...
    def _attempt_stream_palette_load(self):
        ...
        save_object = self.config_object.assembler.save_dict[
            self.stream_sha_from_last_frame
        ].return_stream_header_id()
        ...
        if save_object[0] == True:
            ...
            if (
                save_object[1] in self.config_object.color_handler.custom_palette_list
                or save_object[1]
                in self.config_object.color_handler.default_palette_list
            ):

                self.stream_palette = palette_grabber(save_object[1])
                logging.info(
                    f"Palette ID {save_object[1]} already saved in system... successfully loaded!"
                )

            # This is a new palette which will now be instantiation as a custom palette object.
            else:

                self.fatal_checkpoint = _validate_and_add_palette(
                    save_object[2], save_object[3], save_object[4], save_object[5]
                )

                if self.fatal_checkpoint == False:
                    logging.critical(
                        "Palette for this stream cannot be loaded!  This could only be caused by data "
            ...
            self.frame_handler.update_dictionaries(
                "stream_palette", self.stream_palette_dict, self.stream_palette
            )
            self.config_object.assembler.save_dict[
                self.stream_sha
            ].stream_palette_read = True
    ...
    def _image_frame_setup(self):
        ...
        self.protocol_version, self.primary_palette = read_initializer(
            self.frame_handler.return_initializer(),
            self.block_height,
            self.block_width,
        ...
        self.frame_handler.update_dictionaries(
            "primary_palette", self.primary_palette_dict, self.primary_palette
        )
    ...
    def _video_first_frame_setup(self):
        ...
        self.protocol_version, self.header_palette = read_initializer(
            self.frame_handler.return_initializer(),
            self.block_height,
            self.block_width,
            self.config_object.color_handler.custom_palette_list,
        ...
        self.frame_handler.update_dictionaries(
            "header_palette", self.header_palette_dict, self.header_palette
        )
