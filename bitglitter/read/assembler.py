...
class Assembler:
    ...
    def check_if_frame_needed(self, stream_sha, frame_number):
        ...
        if stream_sha not in self.save_dict:
            ...
            if self.save_dict[stream_sha].is_frame_needed(frame_number) == False:
                logging.info(
                    f"Frame # {frame_number} / {self.save_dict[stream_sha].total_frames} for stream SHA "
    ...
    def create_partial_save(
        ...
        self.save_dict[stream_sha] = PartialSave(
            stream_sha,
            self.working_folder,
            scrypt_n,
            scrypt_r,
    ...
    def save_frame_into_partial_save(self, stream_sha, payload, frame_number):
        logging.info(
            f"Frame # {frame_number} / {self.save_dict[stream_sha].total_frames} for stream SHA {stream_sha} "
            f"saved!"
        )
    ...
    def review_active_sessions(self):
        ...
        if self.active_session_hashes:
            ...
            for partial_save in self.active_session_hashes:
                ...
                if (
                    self.save_dict[partial_save]._attempt_assembly() == False
                ...
                # Assembled, temporary files pending deletion.
                    ...
                    logging.info(
                        f"{partial_save} fully read!  Deleting temporary files..."
                    )
