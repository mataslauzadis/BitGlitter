...
def verify_read_parameters(
    ...
    if (
        os.path.splitext(file_to_input)[1] not in VALID_VIDEO_FORMATS
        and os.path.splitext(file_to_input)[1] not in VALID_IMAGE_FORMATS
    ):
        raise ValueError(
            f"file_to_input argument {file_to_input} is not a valid format.  Only the following are "
