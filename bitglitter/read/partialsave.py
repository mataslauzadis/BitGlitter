...
from bitglitter.protocols.protocol_one.read.protocol_one_postprocess import (
    PostProcessor,
)
from bitglitter.read.partialsaveassets import (
    format_file_list,
    decode_stream_header_ascii_compressed,
    decode_stream_header_binary_preamble,
)
from bitglitter.utilities.filemanipulation import (
    compress_file,
    decompress_file,
    return_hash_from_file,
)
...
class PartialSave:
    ...
    def __init__(
        ...
        self.is_assembled = (
            False  # Did the frames successfully merge into a single binary?
        )
    ...
    def load_frame_data(self, frame_data, frame_number, is_recursive=False):
        ...
        if (
            self.stream_header_ascii_complete == False
            and frame_number == self.next_stream_header_sequential_frame
        ):
        ...
        logging.debug(
            f"Frame {frame_number} for stream {self.stream_sha} successfully saved!"
        )
        ...
        if (
            ...
            frame_data = self._read_file(
                f"frame{self.next_stream_header_sequential_frame}"
            )
            self.load_frame_data(
                frame_data, self.next_stream_header_sequential_frame, is_recursive=True
            )

    def user_input_update(
        self, password_update, scrypt_n, scrypt_r, scrypt_p, change_output_path
    ):
    ...
    def _attempt_assembly(self):
        ...
        if (
        ...
        ):  # If false, assembly will be attempted.  Otherwise, we skip to postprocessing.
            ...
            if self.total_frames == self.frames_ingested:
                ...
                with open(assembled_path, "ab") as assemble_package:
                    ...
                    for frame in range(
                        self.total_frames - self.payload_begins_this_frame + 1
                    ):
    ...
    def _stream_header_assembly(self, frame_data, frame_number):
        ...
        if self.stream_header_preamble_complete == False:
            ...
            self.stream_header_preamble_buffer.append(
                frame_data.read(f"bits:{read_length}")
            )
        ...
        if (
            self.stream_header_ascii_complete == False
            and frame_data.len - frame_data.bitpos > 0
        ):
            # ASCII header rolls over to the next frame.
            if (
                self.ascii_header_byte_size * 8 - self.stream_header_ascii_buffer.len
                >= frame_data.len - frame_data.bitpos
            ):
            ...
            # ASCII header terminates on this frame.
                ...
                read_length = (
                    self.ascii_header_byte_size * 8
                    - self.stream_header_ascii_buffer.len
                )

            self.stream_header_ascii_buffer.append(
                frame_data.read(f"bits:{read_length}")
            )
    ...
    def read_stream_header_binary_preamble(self):
        ...
        (
            ...
            self.custom_palette_used,
            self.date_created,
            self.stream_palette_id,
            self.ascii_header_byte_size,
        ...
        self.date_created = datetime.datetime.fromtimestamp(
            int(self.date_created)
        ).strftime("%Y-%m-%d %H:%M:%S")

        logging.info(
            f"*** Part 1/2 of header decoded for {self.stream_sha}: ***\nPayload size: {self.size_in_bytes} B"
    ...
    def read_stream_header_ascii_compressed(self):
        ...
        (
            ...
            self.file_list,
            self.custom_color_name,
            self.custom_color_description,
            self.custom_color_date_created,
            self.custom_color_palette,
            self.post_compression_sha,
        ) = decode_stream_header_ascii_compressed(
            self.stream_header_ascii_buffer,
            self.custom_palette_used,
            self.encryption_enabled,
        )
        ...
        logging.info(
            f"*** Part 2/2 of header decoded for {self.stream_sha}: ***\nProgram version of sender: "
        ...
        # The stream is not assembled.
            ...
            # The preamble has been loaded, checking the reference table.
                ...
                if self.frame_reference_table == None:
                    ...
                    self.frame_reference_table = BitStream(
                        self._read_file("\\frame_reference_table", to_decompress=True)
                    )
    ...
    def _close_session(self):
        ...
        if self.stream_header_preamble_complete == True:
            self._write_file(
                self.frame_reference_table, "\\frame_reference_table", to_compress=True
            )
    ...
    def return_stream_header_id(self):
        ...
        return (
            self.stream_header_ascii_complete,
            self.stream_palette_id,
            self.custom_color_name,
            self.custom_color_description,
            self.custom_color_date_created,
