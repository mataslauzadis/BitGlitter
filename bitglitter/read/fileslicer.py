    ...
    if is_video:
        ...
        for frame in range(video_frame_puller.total_frames):
            ...
            if bad_frames_this_session < bad_frame_strikes or bad_frame_strikes == 0:
                logging.info(
                    f"Processing frame {video_frame_puller.current_frame} of "
                ...
                if decoder.decode_video_frame(active_frame) == False:
                    if decoder.duplicate_frame_read == False:
                        ...
                        if bad_frame_strikes > 0:
                            logging.warning(
                                f"Bad frame strike {bad_frames_this_session} / {bad_frame_strikes}"
                            )

                        else:
                            logging.warning(
                                f"Bad frame strike # {bad_frames_this_session}"
                            )
                ...
                logging.info(
                    f"Breaking out of video after reaching {bad_frame_strikes} bad frame strikes..."
                )
                break
