...
def frame_lock_on(
    image, block_height_override, block_width_override, frame_width, frame_height
):
    ...
    if block_height_override and block_width_override:  # Jump straight to verification
        logging.info(
            "block_height_override and block_width_override parameters detected.  Attempting to lock with "
            "these values..."
        )
        pixel_width = (
            (frame_width / block_width_override)
            + (frame_height / block_height_override)
        ) / 2

        checkpoint = verify_blocks_x(
            image,
            pixel_width,
        ...
        if return_distance(image[0, 0], (0, 0, 0)) > 100:
            logging.warning(
                "Frame lock fail!  Initial pixel value exceeds maximum color distance allowed for a "
                "reliable lock."
            )
            return False, False, False

        pixel_width, block_dimension_guess = pixel_creep(
            image,
            initializer_palette_a,
            initializer_palette_b,
            combined_colors,
            initializer_palette_a_dict,
            initializer_palette_b_dict,
            frame_width,
...
def verify_blocks_x(
    ...
    for x_block in range(17):
        snapped_value = color_snap(
            scan_block(image, pixel_width, x_block, 0), combined_colors
        )

        if x_block % 2 == 0:
            calibrator_bits_x.append(
                initializer_palette_a_dict.get_value(snapped_value)
            )

        else:
            calibrator_bits_x.append(
                initializer_palette_b_dict.get_value(snapped_value)
            )
    ...
    if read_calibrator_x.read("uint:16") != block_width_estimate:
        if override == True:
            logging.warning(
                "block_width_override is not equal to what was read on calibrator.  Aborting..."
            )

        else:
            logging.warning(
                "block_width verification does not match initial read.  This could be the result of \n"
...
def verify_blocks_y(
    ...
    for y_block in range(17):
        snapped_value = color_snap(
            scan_block(image, pixel_width, 0, y_block), combined_colors
        )

        if y_block % 2 == 0:
            calibrator_bits_y.append(
                initializer_palette_a_dict.get_value(snapped_value)
            )
    ...
    if read_calibrator_y.read("uint:16") != block_height_estimate:
        if override == True:
            logging.warning(
                "block_height_override is not equal to what was read on calibrator.  Aborting..."
            )

        else:
            logging.warning(
                "block_height verification does not match initial read.  This could be the result of \n"
                "sufficiently distorted frames.  Aborting..."
            )
...
def pixel_creep(
    ...
    for value in range(16):
        while True:
            ...
            if (
                active_distance < 100
            ):  # Iterating over same colored blocks, until distance exceeds 100.
                continue
