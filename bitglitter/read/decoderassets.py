...
def minimum_block_checkpoint(
    ...
    if block_height_override and block_width_override:
        if (
            active_frame_size_width < block_width_override
            or active_frame_size_height < block_height_override
        ):
            logging.warning(
                "Block override parameters are too large for a file of these dimensions.  "
                "Aborting..."
            )
            return False
...
def scan_block(image, pixel_width, block_width_position, block_height_position):
    ...
    if pixel_width < 5:
        ...
        start_position_x = int(
            round((block_width_position * pixel_width) + (pixel_width * 0.25), 1)
        )
        end_position_x = int(round(start_position_x + (pixel_width * 0.5), 1))
        start_position_y = int(
            round((block_height_position * pixel_width) + (pixel_width * 0.25), 1)
        )
        end_position_y = int(round(start_position_y + (pixel_width * 0.5), 1))

    numpy_output = numpy.flip(
        image[start_position_y:end_position_y, start_position_x:end_position_x]
    ).mean(axis=(0, 1))
...
def read_initializer(
    bit_stream, blockHeight, block_width, custom_palette_list, default_palette_list
):
    ...
    if str(protocol_version) not in protocol_handler.available_protocols:
        logging.warning(
            f"Protocol v{str(protocol_version)} not supported in this version of BitGlitter.  Please update"
            f" to fix.  Aborting..."
        )
        return False, False
    ...
    if read_block_height != blockHeight or read_block_width != block_width:
        ...
        logging.debug(
            f"read_block_height: {read_block_height}\n blockHeight {blockHeight}"
    ...
    if frame_palette_id > 100:
        ...
        if frame_palette_id not in custom_palette_list:
            ...
            logging.warning(
                "read_initializer: This header palette is unknown, reader cannot proceed until it is "
        ...
        if str(frame_palette_id) not in default_palette_list:
            logging.warning(
                "read_initializer: This default palette is unknown by this version of BitGlitter.  This\n"
                "could be the case if you're using an older version.  Aborting..."
            )
            logging.debug(
                f"frame_palette_id: {frame_palette_id}\ndefault_palette_list: {default_palette_list}"
            )
            return False, False
...
def validate_payload(payload_bits, read_frame_sha):
    ...
    if string_output != read_frame_sha:
        logging.warning(
            "validate_payload: read_frame_sha does not match calculated one.  Aborting..."
        )
        logging.debug(
            f"Read from frameHeader: {read_frame_sha}\nCalculated just now: {string_output}"
        )
        return False
