...
class FrameHandler:
    ...
    def __init__(self, initializer_palette, initializer_palette_dict):
        ...
        # Dictionaries
        self.palette_dict = {
            "initializer_palette": self.initializer_palette,
            "primary_palette": self.primary_palette,
            "header_palette": self.header_palette,
            "stream_palette": self.stream_palette,
        }
        self.palette_conversion_dict = {
            "initializer_palette": self.initializer_palette_dict,
    ...
    def _blocks_to_bits(self, how_many, palette_type):
        ...
        for block in range(how_many):
            ...
            raw_rgb = scan_block(
                self.image, self.pixel_width, block_coords[0], block_coords[1]
            )
            if active_color_set:
                bit_string.append(
                    active_palette_dict.get_value(color_snap(raw_rgb, active_color_set))
                )
    ...
    def return_frame_header(self, palette_type):
        ...
        if (
            palette_type != "stream_palette"
            and palette_type != "header_palette"
            and palette_type != "primary_palette"
        ):
            raise ValueError(
                "FrameHandler.return_frame_header: invalid palette_type argument."
            )

        full_block_data = self._blocks_to_bits(
            math.ceil(608 / self.palette_dict[palette_type].bit_length),
            f"{palette_type}",
        )
        ...
        if full_block_data.len > 608:
            ...
            carry_over_bits.append(
                full_block_data.read(f"bits : {full_block_data.len - 608}")
            )
    ...
    def return_remainder_payload(self, palette_type):
        ...
        if (
            ...
            raise ValueError(
                "FrameHandler.return_remainder_payload: invalid palette_type argument."
            )

        remainder_payload = self._blocks_to_bits(
            self.non_calibrator_blocks - self.block_position, f"{palette_type}"
        )
    ...
    def update_dictionaries(self, palette_type, palette_dict, palette):
        ...
        if palette_type == "header_palette":
            ...
            raise ValueError(
                "FrameHandler.updateDictionary: invalid palette_type argument."
            )
