...
from bitglitter.utilities.generalverifyfunctions import (
    is_bool,
    is_valid_directory,
    is_int_over_zero,
...
def palette_verify(
    header_type, bit_length, block_width, block_height, output_type, fps=0
):
    ...
    if occupied_blocks > total_blocks:
        raise ValueError(
            f"{header_type} has {occupied_blocks - total_blocks} too few blocks with this configuration."
    ...
    return (
        (round(100 - (occupied_blocks / total_blocks * 100), 2)),
        bits_available,
        output_per_sec,
    )


def verify_write_parameters(
    file_list,
    stream_name,
    stream_description,
    stream_output_path,
    ...
    if not file_list:
        raise FileNotFoundError(
            "A minimum of one file or folder is required for stream creation for argument file_list"
            " in write()."
        )
    ...
    if output_mode == "video":
        if header_palette == "24" or stream_palette == "24":
            raise ValueError(
                "24 bit palettes can not be used with videos at this time due to codec issues.  This is"
    ...
    if output_mode != "video" and output_mode != "image":
        raise ValueError(
            "Argument output_mode in write() only accepts 'video' or 'image'."
        )
    ...
    def does_palette_exist(palette, type):
        if palette in config.color_handler.default_palette_list:
            ...
            palette_to_return = config.color_handler.custom_palette_nickname_list[
                palette
            ]
        else:
            raise ValueError(
                f"Argument {type} in write() is not a valid ID or nickname.  Verify that exact value "
                "exists."
            )
    ...
    if frames_per_second != 30 and frames_per_second != 60:
        raise ValueError(
            "frames_per_second must either be 30 or 60 at this time (we're working on this!)"
        )
    ...
    returned_header_values = palette_verify(
        "header_palette",
        active_header_palette.bit_length,
        block_width,
        block_height,
        output_mode,
        frames_per_second,
    ...
    logging.info(
        f"{returned_header_values[0]}% of the frame for initial header is allocated for frame payload (higher "
        f"is better)"
    )

    returned_stream_values = palette_verify(
        "stream_palette",
        active_stream_palette.bit_length,
        block_width,
        block_height,
        output_mode,
        frames_per_second,
    ...
    logging.info(
        f"{returned_stream_values[1]} B, or {returned_stream_values[0]}% of subsequent frames is allocated for"
        f" frame payload (higher is better)"
    )

    if output_mode == "video":
        logging.info(
            f"As a video, it will effectively be transporting {round(returned_stream_values[2] / 8)} B/sec of"
            f" data."
        )
