...
from bitglitter.protocols.protocol_one.write.protocol_one_renderassets import (
    render_calibrator,
    generate_initializer,
    generate_frame_header,
    generate_stream_header_binary_preamble,
    loop_generator,
)
...
def render_loop(
    ...
    frame_number = 1
    primary_frame_palette_dict, primary_read_length = (
        header_palette_dict,
        header_palette.bit_length,
    )
    ...
    stream_header_binary_preamble = generate_stream_header_binary_preamble(
        size_in_bytes,
        total_frames,
        compression_enabled,
        encryption_enabled,
        file_mask_enabled,
        stream_palette.palette_type == "custom",
        date_created,
        stream_palette.id,
        len(ascii_compressed),
    )
    ...
    while active_payload.bitpos != active_payload.length:
        ...
        image = Image.new(
            "RGB", (pixel_width * block_width, pixel_width * block_height), "white"
        )
        ...
        if frame_number == 1 or output_mode == "image":
            ...
            initializer_holder = generate_initializer(
                block_height, block_width, protocol_version, active_palette
            )
            initializer_palette_blocks_used += INITIALIZER_DATA_BITS
            blocks_left -= INITIALIZER_OVERHEAD
            initializer_enabled = True

        bits_left_this_frame = (
            blocks_left * active_palette.bit_length
        ...
        # Frames that need stream_header_combined added to them.
            ...
            # stream_header_combined terminates on this frame
                ...
                stream_header_chunk = stream_header_combined.read(
                    len(stream_header_combined) - stream_header_combined.bitpos
                )
                ...
                stream_header_blocks_used = math.ceil(
                    bits_consumed / active_palette.bit_length
                )
                ...
                if final_block_partial_fill > 0:
                    attachment_bits = active_palette.bit_length - (
                        final_block_partial_fill
                    )
                ...
                if remaining_blocks_left > 0:
                    bits_left_this_frame = (
                        remaining_blocks_left * active_palette.bit_length
                    )

                    if (
                        max_allowable_payload_bits > bits_left_this_frame
                    ):  # Payload continues on in next frame(s)
                        remainder_blocks_into_bits = active_payload.read(
                            bits_left_this_frame
                        )

                    else:  # Full frame_payload can terminate on streamHeader termination frame.
                        remainder_blocks_into_bits = active_payload.read(
                            max_allowable_payload_bits
                        )
                        last_frame = True

        frame_hashable_bits = (
            stream_header_chunk
            + attachment_bits_append
            + remainder_blocks_into_bits
            + payload_holder
        )
        combined_frame_length = frame_hashable_bits.len + FRAME_HEADER_OVERHEAD
        blocks_used = (int(initializer_enabled) * INITIALIZER_DATA_BITS) + math.ceil(
            combined_frame_length / active_palette.bit_length
        )
        ...
        if last_frame == True:
            ...
            remainder_bits = active_palette.bit_length - (
                combined_frame_length % active_palette.bit_length
            )
        ...
        frame_header_holder = generate_frame_header(
            stream_sha, frame_hashable_bits, frame_number, blocks_used
        )
        combining_bits = (
            initializer_holder
            + frame_header_holder
        ...
        next_coordinates = loop_generator(
            block_height, block_width, pixel_width, initializer_enabled
        )
        block_position = 0
        ...
        while len(all_bits_to_write) != all_bits_to_write.bitpos:
            ...
            if block_position >= initializer_palette_blocks_used:
                active_palette_dict, read_length = (
                    primary_frame_palette_dict,
                    primary_read_length,
                )
            ...
            # Initializer palette selection
                ...
                active_palette_dict, read_length = (
                    initializer_palette_dict,
                    initializer_palette.bit_length,
                )

            # Here to signal something has broken.
            else:
                raise Exception(
                    "Something has gone wrong in matching block position to palette.  This state"
        ...
        if output_mode == "video":
            file_name = frame_number_to_string.zfill(
                math.ceil(math.log(total_frames + 1, 10))
            )
            ...
            if output_name:
                ...
                file_name = (
                    time.strftime("%Y-%m-%d %H-%M-%S", time.localtime(date_created))
                    + " - "
                    + str(frame_number)
                )
