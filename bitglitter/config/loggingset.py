...
def logging_setter(logging_level, logging_print: bool, logging_save_output: bool):
    ...
    if logging_level not in acceptable_level_words:
        raise ValueError(
            f"{logging_level} is not a valid input for loggingLevel.  Only 'info', 'debug', and False are "
            f"allowed."
        )
    ...
    if logging_save_output:
        ...
        output_log_formatter = logging.Formatter(
    ...
    if logging_print:
        ...
        print_log_formatter = logging.Formatter(
