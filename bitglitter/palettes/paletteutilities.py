    ...
    def generate_dictionary(self):
        ...
        if self.palette.bit_length != 24:
            ...
            for value in range(len(self.palette.color_set)):
                ...
                temp_bin_holder = str(
                    BitArray(uint=value, length=self.palette.bit_length)
                )
...
def _validate_and_add_palette(
    palette_name, palette_description, date_created, color_set
):
    ...
    _add_custom_palette_direct(
        palette_name, palette_description, color_set, distance, date_created, id
    )
...
def return_palette_id(name, description, date_created, color_set):
    ...
    hasher = hashlib.sha256(
        str(name + description + date_created + color_set_string).encode()
    )
    return hasher.hexdigest()


def _add_custom_palette_direct(
    name, description, color_set, distance, date_created, id, nickname=None
):
    ...
    new_palette = CustomPalette(
        name, description, color_set, distance, date_created, id, nickname
    )
