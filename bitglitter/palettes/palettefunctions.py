...
def add_custom_palette(
    palette_name, palette_description, color_set, optional_nickname=""
):
    ...
    if (
        optional_nickname in config.color_handler.custom_palette_nickname_list
        or optional_nickname in config.color_handler.custom_palette_list
        or optional_nickname in config.color_handler.default_palette_list
    ):
        raise ValueError(
            f"'{optional_nickname}' is already taken, please choose another nickname."
        )
    ...
    for color_tuple in color_set:
        ...
        for color in color_tuple:
            if not isinstance(color, int) or color < 0 or color > 255:
                raise ValueError(
                    "For each RGB value, it must be an integer between 0 and 255."
                )

    min_distance = color_distance(color_set)
    if min_distance == 0:
        raise ValueError(
            "Calculated color distance is 0.  This occurs when you have two identical colors in your"
...
def edit_nickname_to_custom_palette(id_or_nick, new_name):
    ...
    if (
        new_name not in config.color_handler.custom_palette_list
