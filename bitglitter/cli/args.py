...
parser = argparse.ArgumentParser(
    description=""""Embed data payloads inside of ordinary
...
write_parser.add_argument(
    "-file", type=str, help="The file in which data will be written."
)

write_parser.add_argument("-streamname", type=str, help="Stream name.")

write_parser.add_argument(
    "-mode", type=str, help='File output mode: "image" or "video".'
)
write_parser.add_argument("-o", type=str, help="File output path.")
...
read_parser.add_argument("-file", type=str, help="File to be decoded.")

read_parser.add_argument("-o", type=str, help="File to be decoded.")


cli_argument = vars(parser.parse_known_args()[0]).get("subparser_name")
if cli_argument == "read":
    read_parser.set_defaults(
        func=CliArgumentParser.read(vars(read_parser.parse_known_args()[0]))
    )
elif cli_argument == "write":
    write_parser.set_defaults(
        func=CliArgumentParser.write(vars(write_parser.parse_known_args()[0]))
    )
