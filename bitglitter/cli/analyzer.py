    ...
    def read(arguments: dict):
        ...
        if path.isfile(file) == True or path.isdir(file) == True:
            ...
            raise FileNotFoundError(
                "Read first argument must be a valid path to a file or folder."
            )
    ...
    def write(arguments: dict):
        ...
        if path.isfile(file_path) == True or path.isdir(file_path) == True:
            try:
                ...
                raise FileNotFoundError(
                    "ffmpeg is required and wasn't found do you have ffmpeg installed in your local directory ?"
                )
            except TypeError:
                raise TypeError("Please provide -streamname argument.")
            except PermissionError:
                raise PermissionError(
                    f"You don't have the proper permissions to write at {output_path}, \ntry to change output path or run the command with sudo under Linux"
                )
        else:
            raise FileNotFoundError(
                "Write first argument must be a valid path to a file or folder."
            )
